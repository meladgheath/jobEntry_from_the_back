-- CreateTable
CREATE TABLE "accountNumber" (
    "id" INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
    "number" TEXT NOT NULL,
    "name" TEXT NOT NULL,
    "detail" TEXT NOT NULL
);

-- CreateTable
CREATE TABLE "management" (
    "id" INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
    "code" TEXT NOT NULL,
    "name" TEXT NOT NULL,
    "type" TEXT NOT NULL
);

-- CreateTable
CREATE TABLE "restrictions" (
    "id" INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
    "resID" TEXT NOT NULL,
    "name" TEXT NOT NULL,
    "account" TEXT NOT NULL,
    "accountName" TEXT NOT NULL DEFAULT '',
    "debit" DECIMAL NOT NULL DEFAULT 0,
    "credit" DECIMAL NOT NULL DEFAULT 0,
    "rest_date" DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "managementID" INTEGER NOT NULL,
    CONSTRAINT "restrictions_managementID_fkey" FOREIGN KEY ("managementID") REFERENCES "management" ("id") ON DELETE RESTRICT ON UPDATE CASCADE
);

-- CreateTable
CREATE TABLE "users" (
    "username" TEXT NOT NULL PRIMARY KEY,
    "name" TEXT NOT NULL,
    "password" TEXT NOT NULL
);

-- CreateTable
CREATE TABLE "store" (
    "serailNumber" TEXT NOT NULL PRIMARY KEY,
    "name" TEXT NOT NULL,
    "code" TEXT NOT NULL,
    "managementID" INTEGER NOT NULL,
    "imp_date" DATETIME NOT NULL,
    CONSTRAINT "store_managementID_fkey" FOREIGN KEY ("managementID") REFERENCES "management" ("id") ON DELETE RESTRICT ON UPDATE CASCADE
);

-- CreateTable
CREATE TABLE "store_transactions" (
    "id" INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
    "serailNumber" TEXT NOT NULL,
    "fromStore" INTEGER NOT NULL,
    "toStore" INTEGER NOT NULL
);
